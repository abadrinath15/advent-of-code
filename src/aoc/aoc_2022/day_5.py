...
def row_to_instruction(
    row: str, stacks: list[deque[str]], pop_bunch: bool = False
) -> None:
    ...
    to_s -= 1
    if pop_bunch:
        stacks[from_s].rotate(how_many)
    while how_many > 0:
        if pop_bunch:
            stacks[to_s].append(stacks[from_s].popleft())
        else:
...
def main(input_fp: str, pop_bunch: bool = False) -> str:
    ...
    with open(input_fp) as file:
        ...
        for line in file_iter:
            row_to_instruction(line, stacks, pop_bunch)
...
if __name__ == "__main__":
    ...
    print(main("src/aoc/aoc_2022/inputs/day_5.txt", pop_bunch=True))
